...
void openMap(string location, bool fromScript)
{
    ...
    while (!WindowShouldClose())
    {
        ...
        if (IsKeyPressed(KeyboardKey.KEY_TAB) || IsGamepadButtonPressed(gamepadInt, GamepadButton.GAMEPAD_BUTTON_LEFT_TRIGGER_1) || IsGamepadButtonPressed(
                gamepadInt, GamepadButton.GAMEPAD_BUTTON_LEFT_TRIGGER_2))
        {
        ...
        if (IsKeyPressed(KeyboardKey.KEY_DOWN) || IsGamepadButtonPressed(gamepadInt, GamepadButton
                .GAMEPAD_BUTTON_LEFT_FACE_DOWN))
        {
            selectedMenuIndex = cast(int)(
                (selectedMenuIndex + 1) % currentLocationData.menuOptions.length);
        }
        if (IsKeyPressed(KeyboardKey.KEY_UP) || IsGamepadButtonPressed(gamepadInt, GamepadButton
                .GAMEPAD_BUTTON_LEFT_FACE_UP))
        {
            selectedMenuIndex = cast(int)(
                (selectedMenuIndex - 1 + currentLocationData.menuOptions.length) % currentLocationData
                    .menuOptions.length);
        }
...
void drawUI(int screenWidth, int screenHeight, string currentArea, LocationData currentLocationData, int selectedMenuIndex, Texture2D mapTextureAkenadai, Texture2D mapTextureShibahama, Texture2D arrowTexture)
{
    ...
    DrawTextEx(fontdialog, toStringz((currentArea == "Akenadai") ? "Shibahama" : "Akenadai"), Vector2(
            screenWidth / 2 + 20, 10), 30, 0, Colors.GRAY);
    ...
    for (int i = 0; i < currentLocationData.menuOptions.length; i++)
    {
        ...
        DrawTextEx(fontdialog, toStringz(currentLocationData.menuOptions[i]), Vector2(20, 64 + (
                30 * i)), 20, 1.0f, Colors.WHITE);
    }
    DrawRectangleRoundedLinesEx(Rectangle(10, 60, 200, 30 * currentLocationData.menuOptions.length), 0.03f, 16, 3.0f, Color(
            100, 54, 65, 255)); // Outline
...
{
    if (IsKeyPressed(KeyboardKey.KEY_ENTER) || IsGamepadButtonDown(gamepadInt, GamepadButton
            .GAMEPAD_BUTTON_RIGHT_FACE_DOWN))
    {
        ...
        while (fadeOutAlpha < 255)
        {
            ...
            DrawRectangleRoundedLinesEx(Rectangle(10, 60, 200, 30 * currentLocationData
                    .menuOptions.length), 0.03f, 16, 3.0f, Color(100, 54, 65, 255)); // Outline
...
Texture2D[] loadArrowTextures()
{
    ...
    {
        uint arrowSize;
        char* arrow_data = get_file_data_from_archive("res/data.bin", toStringz(
                "MC*-" ~ i.to!string() ~ ".png"), &arrowSize);
...
void drawMapTexture(int screenWidth, int screenHeight, Texture2D mapTextureAkenadai, Texture2D mapTextureShibahama, string currentArea)
{
    ...
    {
        DrawTexturePro(mapTextureShibahama, Rectangle(0, 0, cast(float) mapTextureShibahama.width, cast(
                float) mapTextureShibahama.height), Rectangle(0, 0, cast(float) screenWidth, cast(
                float) screenHeight), Vector2(0, 0), 0.0, Colors.WHITE);
    }
    else if (currentArea == "Akenadai")
    {
        DrawTexturePro(mapTextureAkenadai, Rectangle(0, 0, cast(float) mapTextureAkenadai.width, cast(
                float) mapTextureAkenadai.height), Rectangle(0, 0, cast(float) screenWidth, cast(
                float) screenHeight), Vector2(0, 0), 0.0, Colors.WHITE);
    }
}