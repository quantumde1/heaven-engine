...
void loadAssets(string[] demons_filenames)
{
    ...
    for (int i = 0; i < randomNumber; i++)
    {
        ...
        if (!isBossfight)
            demon_data = parseJSON(readText(
                    "res/enemies_data/" ~ demons_filenames[demonNumber[i]] ~ ".json"));
        else
            demon_data = parseJSON(
                readText("res/enemies_data/" ~ demonsBossfightAllowed[i] ~ ".json"));
        uint image_size;
...
void drawEnemies()
{
    ...
    DrawTexturePro(background, Rectangle(0, 0, cast(float) background.width, cast(float) background
            .height),
    ...
    for (long i = 0; i < numberOfEnemies; i++)
    {
        if (enemies[i].currentHealth > 0)
        {
            ...
            if (i == selectedEnemyIndex && selectingEnemy)
            {
                ...
                enemyColor = Color(cast(ubyte) 255, cast(ubyte) 255, cast(ubyte) 0, cast(ubyte) cast(
                        int)(alpha * 255));
            }
            ...
            Vector2 finalPosition = Vector2(posX - textureHalfWidth + enemies[i].shakeOffset.x, -200 + posY - textureHalfHeight + enemies[i]
                    .shakeOffset.y);
...
void drawBattleMenu()
{
    ...
    if (!isPlayingAnimation && !drawBattleUI)
    {
        ...
        textY += 50;
        DrawTextEx(fontdialog, toStringz("Achieved XP: " ~ to!string(addXP)), Vector2(rectX + 30, textY), 30, 1.0f, Colors
                .WHITE);
        textY += 40;

        DrawTextEx(fontdialog, toStringz("Current XP: " ~ to!string(XP + addXP)), Vector2(rectX + 30, textY), 30, 1.0f, Colors
                .WHITE);
...
void handleMenuInput(int numberOfButtons, int numberOfTabs)
{
    ...
    case 0: // Attack
            ...
            || IsGamepadButtonPressed(gamepadInt, GamepadButton.GAMEPAD_BUTTON_RIGHT_FACE_DOWN) && selectedTabIndex == 0 && !selectingEnemy)
        {
            ...
            || selectingEnemy && IsGamepadButtonPressed(gamepadInt, GamepadButton
                .GAMEPAD_BUTTON_RIGHT_FACE_DOWN))
        {
            debug debug_writeln("enter pressed for acting");
            debug debug_writeln("current selected enemy(", selectedEnemyIndex, ") hp before attack is ", enemies[selectedEnemyIndex]
                    .currentHealth);
            performPhysicalAttack(selectedEnemyIndex);
            debug debug_writeln("current selected enemy(", selectedEnemyIndex, ") hp after attack is ", enemies[selectedEnemyIndex]
                    .currentHealth);
    ...
    if (selectingEnemy)
    {
        ...
        if (IsKeyPressed(KeyboardKey.KEY_LEFT) || (IsGamepadButtonPressed(gamepadInt, GamepadButton
        ...
        if (IsKeyPressed(KeyboardKey.KEY_RIGHT) || (IsGamepadButtonPressed(gamepadInt, GamepadButton
...
void performPhysicalAttack(int enemyIndex)
{
    if (enemies[enemyIndex].currentHealth > 0)
    {
        ...
        debug debug_writeln("attacked enemy no.", enemyIndex, "! HP of enemy is ", enemies[enemyIndex].currentHealth, " player HP is ", partyMembers[0]
                .currentHealth);
        if (enemies[enemyIndex].currentHealth <= 0)
        {
...
void enemyTurn()
{
    ...
    for (int i = 0; i < enemies.length; i++)
    {
        ...
        if (enemies[i].currentHealth > 0)
        {
            ...
            JSONValue demon_data = parseJSON(readText(
                    "res/enemies_data/" ~ demonsAllowed[demonNumber[i]] ~ ".json"));
