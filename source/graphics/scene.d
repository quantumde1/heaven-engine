...
void parseSceneFile(string path)
{
    ...
    for (int i = 0; i < objects.array.length; i++)
    {
        ...
        case "model":
            ...
            modelPosition ~= Vector3(position["x"].get!float, position["y"].get!float, position["z"]
                    .get!float);
            modelLocationSize ~= Vector3(scale["x"].get!float, scale["y"].get!float, scale["z"]
                    .get!float);
            modelLocationRotate ~= Vector3(rotation["x"].get!float, rotation["y"].get!float, rotation["z"]
                    .get!float);
        ...
        case "light":
            ...
            light_pos ~= LightEngine(Vector3(position["x"].get!float, position["y"].get!float,
                    position["z"].get!float), Color(color["r"].get!int
                    .to!ubyte, color["g"].get!int
                    .to!ubyte,
                    color["b"].get!int
                    .to!ubyte, color["a"].get!int
                    .to!ubyte));
            break;
    ...
    Model[] cubeModels, Model playerModel)
{
    ...
    if (shaderEnabled)
        SetShaderValue(shader, shader.locs[ShaderLocationIndex.SHADER_LOC_VECTOR_VIEW], &cameraPos[0], ShaderUniformDataType
                .SHADER_UNIFORM_VEC3);
    BeginMode3D(camera);
    ...
    foreach (i, cubeModel; cubeModels)
    {
        ...
        DrawModelEx(cubeModel, position, Vector3(0.0f, 1.0f, 0.0f), cubes[i].rotation, Vector3(modelCubeSize, modelCubeSize, modelCubeSize), Colors
                .WHITE);
    }
    ...
    if (isMoving == true)
    {
        playerRotation = (-cameraAngle * std.math.PI / 180.0f) + additionalRotation + playerModelRotation * std
            .math.PI / 180.0f;
    ...
    {
        DrawModelEx(floorModel[i], modelPosition[i], modelLocationRotate[i], rotateAngle[i], modelLocationSize[i], Colors
                .WHITE);
    }
    if (showDebug)
        drawDebugCollisions();
