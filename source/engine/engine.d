    ...
    public:
        ...
        void run() {    
            ...
            KeyAction[] keyActionsPlayer = [
                {forward, () { 
                    player.coordinates.z -= player.speed;
                    scene.camera.position.z -= player.speed;
                    scene.camera.target.z -= player.speed;
                }},
                {backward, () { 
                    player.coordinates.z += player.speed;
                    scene.camera.position.z += player.speed;
                ...
                {left, () { 
                    ...
                    scene.camera.position.x -= player.speed;
                    scene.camera.target.x -= player.speed;
                }},
                {right, () { 
                    player.coordinates.x += player.speed;
                    scene.camera.position.x += player.speed;
