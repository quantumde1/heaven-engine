...
void drawMoonState(int rectX, int rectY, int rectWidth, int rectHeight, Font fontdialog)
{
    ...
    DrawTextEx(fontdialog, moonPhases[currentPhase].ptr, Vector2(rectX + 10, rectY + 40), 20, 1.0f, Colors
            .WHITE);
}
...
void drawCompass(const int compassSize, const int compassX, const int compassY, const int centerX, const int centerY, float cameraAngle, Font navFont)
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        Vector2 dirPos = Vector2(centerX + directions[i].x * (halfCompassSize - 20), centerY - directions[i].y * (
                halfCompassSize - 20));
        DrawTextEx(navFont, labels[i], Vector2(int(dirPos.x.to!int - MeasureText(labels[i], 20) / 2), int(
                dirPos.y.to!int - 10)), 20, 1.0f, labelColors[i]);
    }
}
...
void draw_navigation(float cameraAngle, Font navFont, Font fontdialog)
{
    ...
    DrawRectangleRoundedLinesEx(Rectangle(cast(int)(centerX - halfCompassSize), cast(int)(
            centerY - halfCompassSize), 200, 270), 0.03f, 16, 5.0f, Color(100, 54, 65, 255));
