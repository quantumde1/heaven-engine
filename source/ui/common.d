...
void drawPlayerHealthAndManaBar(int playerHealth, int maxPlayerHealth, int playerMana, int maxPlayerMana, float xOffset, float yOffset, float barWidth, float barHeight, string playerName)
{
    ...
    else
    {
        ...
        DrawTextEx(fontdialog, toStringz(playerName), Vector2(nameTextX, nameTextY), 20, 1.0f, Colors
                .WHITE);
        DrawRectangleRounded(Rectangle(xOffset + 5, yOffset + 30, cast(int)(
                (barWidth - 10) * healthPercentage), barHeight - 10), 0.03f, 16, Colors.RED);
        DrawRectangleRounded(Rectangle(xOffset + 5, yOffset + barHeight + 30, cast(int)(
                (barWidth - 10) * manaPercentage), barHeight - 10), 0.03f, 16, Colors.BLUE);
        string healthText = "HP: " ~ to!string(playerHealth) ~ "/" ~ to!string(maxPlayerHealth);
        DrawTextEx(fontdialog, toStringz(healthText), Vector2(xOffset + 10, yOffset + 30), 20, 1.0f, Colors
                .WHITE);
        string manaText = "MP: " ~ to!string(playerMana) ~ "/" ~ to!string(maxPlayerMana);
        DrawTextEx(fontdialog, toStringz(manaText), Vector2(xOffset + 10, yOffset + barHeight + 30), 20, 1.0f, Colors
                .WHITE);
    }
}
